...
```powershell
cd 工具
python -m tools.paddleocr_client "..\电子资源\split\27张宇基础30讲（高数）-按章修正版\01-第1讲-函数极限与连续.pdf" --mode async --output-dir "..\张宇基础30讲\paddleocr\_source\01-第1讲-函数极限与连续"
```
...
```powershell
cd 工具
python -m tools.paddleocr_client "path\to\file.pdf" --mode sync --output-dir "output"
```
...
```powershell
cd 工具
python -m tools.paddleocr_client "path\to\page.png" --mode sync --output-dir "output"
```
